(function (factory) {
	...
	var Editor = Edit.Editor = function(opt) {
		...
		this.commands = {};
		this.buttons = {};
		this.options = utils.extend(EDITOR_CONFIG, opt),
	...
	Editor.prototype = {
		...
		setup: function(doc) {
			...
			self.isReady = 1;
			self.emit('ready');
			setTimeout(function(){self.options.initialContent && self.setContent(self.options.initialContent);},0);
		},
		// 获取 html内容
		getContent: function() {
			var self = this;
		...
		initEvents: function() {
			var self = this,
				doc = self.document,
				win = self.window;
			self.bind(doc, ['click', 'dblclick', 'keydown', 'keyup', 'keypress', 'mousedown', 'mouseup', 'mouseover', 'mouseout', 'selectstart'], self.proxyDomEvent.bind(self));
			...
			self.on('keydown',function(ev){
				if (ev.keyCode == 13) {
					if (self.matchSelector(self.getRangeElem('pre'),'pre')) {
						...
						if (self.getRangeElem().nodeName !== 'P') {
							ev.preventDefault();
							self.execCommand('inserthtml','<p><br></p>');
						}
					}					
			...
			self.on('keyup', function(ev){
				if (ev.keyCode == 8 && !self.body.innerHTML) {
			...
			self.on('mouseup', function(){
				if (self.matchSelector(self.getRangeElem('pre'),'pre')) {
					Edit.utils.each(self.buttons,function(v,k){
						if (v != self.buttons['insertcode']) {
							v.setDisabled(true);
						}							
					});
				} else if (self.buttons.code && !self.buttons.code.isChecked()) {
					Edit.utils.each(self.buttons,function(v,k){
						v.setDisabled(false);
					});
				} 
