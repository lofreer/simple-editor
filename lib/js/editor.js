(function (factory) {
	...
	UI.Menu.prototype = {
		getHtmlTpl: function() {
			var self = this;
			return UI.createEl('div',{id:'editor-fixedlayer',class:'editor-fixedlayer'},[self.content],self.handles);
		},
		show: function(fn) {
			var dom;
	...
	utils.extend(UI.Menu.prototype, UI.Stateful);
	...
	UI.Popup.prototype = {
		getHtmlTpl: function() {
			var self = this;
			return UI.createEl('div',{id:self.id,class:'editor-popup'},[self.content],self.handles);
		},
		show: function(el) {
			var dom;
	...
	Editor.prototype = {
		...
		render: function(container) {
			...
			if (!container) return;
			self.options.initialContent = options.initialContent || container.textContent;
			container.innerHTML = '';
	...
	Edit.commands['inserthtml'] = {
		execCommand: function(command, html) {
			var self = this;
			self.document.execCommand('inserthtml',false,html+'<p><br></p>');
		}
	}	
    ...
    ['insertcode'].forEach(function(cmd){
    	Edit.ui[cmd] = function(editor) {    		
			...
			function insertCode() {
				var href = document.getElementById('input-code').value;
				if (href) {
				    		...
				    		content: ce('div',{class:'eui-dialog',style:Edit.ui.getPanelOffset(self.id)},[
				    			ce('div',{class:'panel-box'},[					    			
					    			ce('div',{class:'panel-content'},[	
				    					ce('textarea',{id:'input-code',class:'input-code'}),
				    					ce('div',{class:'panel-handles'},[
				    						ce('span',{class:'panel-cancel'},['取消'],{click:function(){Edit.ui.closePopup()}}),
				    						ce('span',{class:'panel-confirm'},['确定'],{click:insertCode})					    					
					    				])
					    			])
					    		])
				    		])
