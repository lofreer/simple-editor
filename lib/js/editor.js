(function (factory) {
	...
	// 自闭合元素
	var $empty = {area:1,base:1,basefont:1,br:1,col:1,command:1,dialog:1,embed:1,hr:1,img:1,input:1,isindex:1,keygen:1,link:1,meta:1,param:1,source:1,track:1,wbr:1};
	...
	Editor.prototype = {
		...
		initEvents: function() {
			var self = this,
				doc = self.document,
				win = self.window;
			self.bind(doc, ['click', 'dblclick', 'keydown', 'keyup', 'keypress', 'mousedown', 'mouseup', 'mouseover', 'mouseout', 'selectstart', 'paste'], self.proxyDomEvent.bind(self));
			...
			self.on('keydown',function(ev){
				if (ev.keyCode === 13) {
					...
					} else if (self.getRangeElem().nodeName !== 'P') {
						ev.preventDefault();
						self.execCommand('inserthtml','<p><br></p>');
					}			
				} 
		...
		contentChange: function() {
			...
			function handler() {
				...
				if (caches[self.key].value === val) return;
				self.emit('contentchange');
				caches[self.key].value = val;
			}

			self.on('keyup paste', function(ev){
				clearTimeout(timeoutId);
		...
		domFilter: function() {
			var self = this;
			var html = self.body.innerHTML;
			self.body.innerHTML = self.body.innerHTML.replace(/\n/g, '<br>');
    ...
    ['insertcode'].forEach(function(cmd){
    	Edit.ui[cmd] = function(editor) {    		
    		editor.commands[cmd] = {
				...
				queryCommandState: function(c,v) {
					return editor.matchSelector(editor.getRangeElem('pre'),'pre');
				}
			};
    ...
    ['quotes'].forEach(function(cmd){
    	Edit.ui[cmd] = function(editor) {    		
    		editor.commands[cmd] = {
				...
				queryCommandState: function(c,v) {
					return editor.matchSelector(editor.getRangeElem('blockquote'),'blockquote');
				}
			};
